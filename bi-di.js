...
function runBiDijkstra(adj_list, edge_hash, start, end, cost_field) {
  ...
  const searchForward = doDijkstra(adj_list, edge_hash, forward, start, cost_field, 'forward', backward);
  const searchBackward = doDijkstra(adj_list, edge_hash, backward, end, cost_field, 'backward', forward);
  ...
  } while ((forward.dist[sf.value] + backward.dist[sb.value]) < tentative_shortest_path);
  ...
  if(save_output) {
    ...
    fs.writeFile('./bidi_forward_radius.geojson', JSON.stringify(fradius), 'utf8');
    fs.writeFile('./bidi_backward_radius.geojson', JSON.stringify(bradius), 'utf8');
  }
  ...
  function* doDijkstra(graph, edge_hash, ref, current, cost_field, direction, reverse_ref) {
